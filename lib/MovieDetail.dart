import 'package:data_print/moviemodel.dart';
import 'package:flutter/material.dart';
import 'moviemodel.dart';
class MovieDetail extends StatelessWidget{
  final Movie movier;
  const MovieDetail({Key? key, required this.movier}) : super(key: key);
  @override
  Widget build(BuildContext context) {
    // TODO: implement build
    return MaterialApp(
      home: Scaffold(
        appBar: AppBar(title: Text(movier.title),backgroundColor: Colors.deepOrange,centerTitle: true,),
        body: Container(
          margin: EdgeInsets.only(left: 30,right: 30),
          child: Column(
            children: [
              Padding(padding: EdgeInsets.all(20)),
              Image.network(movier.imageUrl,height: 300,width: 400,),
              Text("ID: ${movier.id}",style: TextStyle(fontSize: 20),),
              Text("Title: ${movier.title}",style: TextStyle(fontSize: 20),),
              Text("duration: ${movier.duration}",style: TextStyle(fontSize: 18),),
              Text("Year: ${movier.year}",style: TextStyle(fontSize: 16),),
              Text("Rating: ${movier.rating}",style: TextStyle(fontSize: 14),),


              Text("Description: ${movier.description}",textAlign: TextAlign.justify,),
